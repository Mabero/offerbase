...
function extractTopic(title: string): string | null {
  ...
  console.log('Widget DEBUG: Original title for extraction:', title);
  ...
  for (const pattern of fullTitlePatterns) {
    ...
    console.log('Widget DEBUG: Testing pattern', pattern, 'Result:', match);
    if (match && match[1]) {
      ...
      console.log('Widget DEBUG: Extracted topic from full title:', topic);
  ...
  console.log('Widget DEBUG: Cleaned title:', cleanTitle);
  ...
  for (const pattern of patterns) {
    ...
    console.log('Widget DEBUG: Testing cleaned pattern', pattern, 'Result:', match);
    if (match && match[1]) {
      const topic = match[1].trim()
        .replace(/\b(?:the|a|an|and|or|but|in|on|at|to|for|of|with|by|din|til|alt|innen|om|og|i)\b/gi, ' ') // Remove common words
        .replace(/\s+/g, ' ') // Clean up multiple spaces
      ...
      console.log('Widget DEBUG: Extracted topic from cleaned title:', topic);
      if (topic.length > 3) { // Only return meaningful topics
        return topic.toLowerCase();
      }
    }
  ...
  console.log('Widget DEBUG: No topic extracted');
  return null;
}
